...
- **`@eslint/js` moves with `eslint`, and Dependabot proposes them separately.** Its major tracks eslint's, and from `@eslint/js` 10 the dependency on a matching eslint is declared as an optional peer, which npm neither fails nor warns on. A skew is therefore silent, and it is `js.configs.recommended` in `eslint.config.js` that quietly comes from the wrong major. Merge the two bumps together, and expect a major to add rules: `@eslint/js` 10 added `preserve-caught-error` to the recommended set.
- **`eslint-plugin-jsx-a11y` is held to the root's eslint by an `overrides` entry**, because its peer range stops at eslint 9 while the repository is on 10, and `npm ci` refuses that tree. The plugin itself works on eslint 10; upstream's own fix ([jsx-eslint/eslint-plugin-jsx-a11y#1081](https://github.com/jsx-eslint/eslint-plugin-jsx-a11y/pull/1081)) only widens the range, but it is unmerged and the last release was October 2024. The override reads `$eslint`, which follows whatever the manifest asks for, so a later eslint bump needs no edit here. **Nothing reports that it is there**: npm does not record an override in `package-lock.json`, and no check fails once it has stopped being necessary. Delete it when a jsx-a11y release accepts eslint 10, and let `npm ci` enforce the peer range again.
